...
def validate_manual_date(date_str: str) -> Tuple[bool, Optional[str], Optional[str]]:
    ...
    # Поддерживаемые форматы дат в порядке приоритета
    # Сначала проверяем форматы с точками (наиболее распространенные в России)
    # затем с дефисами и ISO формат, последние - с слешами
    formats = [
        '%d.%m.%Y',    # 29.05.2025 - полный год с точками
        '%d.%m.%y',    # 29.05.25 - короткий год с точками
        '%d-%m-%Y',    # 29-05-2025 - полный год с дефисами
        '%d-%m-%y',    # 29-05-25 - короткий год с дефисами
        '%Y-%m-%d',    # 2025-05-29 - ISO 8601 формат
        '%d/%m/%Y',    # 29/05/2025 - полный год со слешами
        '%d/%m/%y'     # 29/05/25 - короткий год со слешами
    ]

    # Пытаемся распарсить дату, перебирая все поддерживаемые форматы
    # Используем паттерн "попробуй и забудь" - первый успешный парсинг побеждает
    parsed_date = None
    for date_format in formats:
        try:
            parsed_date = datetime.datetime.strptime(date_str, date_format).date()
            break  # Прерываем цикл при первом успешном парсинге
        except ValueError:
            # Формат не подошел, пробуем следующий
            continue

    if parsed_date is None:
        # Ни один формат не подошел - возвращаем ошибку с подсказкой
        return False, None, (
    ...
    # Валидация 1: проверяем, что дата не в будущем
    # Это важно, так как нельзя делать записи "заранее"
    if parsed_date > today:
        return False, None, "Нельзя добавлять записи на будущие даты."

    # Валидация 2: проверяем, что дата не слишком старая
    # Ограничение в 2 года (730 дней) предотвращает случайные опечатки
    # и гарантирует релевантность данных для анализа
    ...
    # Все проверки пройдены - возвращаем дату в нормализованном формате ISO (YYYY-MM-DD)
    # Этот формат используется для хранения в БД и обеспечивает корректную сортировку
